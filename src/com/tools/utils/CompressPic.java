...
public class CompressPic {
	...
	public String compressPic() {
		try {
			...
			if (img.getWidth(null) == -1) {
				...
				dg.drawImage(img.getScaledInstance(newWidth, newHeight, Image.SCALE_SMOOTH), 0, 0, null);
	 ...
	 * @param inputFileName 源图片名
	 * @param outputFileName 目标图片名
	 * @param width 输出图片宽
	 * @param height  输出图片高
	 * @param color  输出图片背景色
	 * @param gp true:等比，false:非等比
	 * @return
	 */
	public String compressPic(String inputDir, String outputDir,
			String inputFileName, String outputFileName, int width, int height,Color color,
			boolean gp) {
		// 输入图路径
		this.inputDir = inputDir;
		// 输出图路径
